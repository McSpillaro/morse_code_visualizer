...
// Processes the logic behind the morse code input patterns. Checks for validity of input pattern (i.e. '..-.') as well as calculates short or long presses.
class MorseCode
{
private:
    ...
    // Handles when the user is done inputting morse code and will process the pattern.
    void handleFinalRelease()
    {
...
public:
    ...
    bool add_input(char *array, char newChar)
    {
        ...
        if (length < Data::MAX_INPUT_SIZE - 2)
        {                             // Ensures there is space for the new character and null terminator
    ...
    bool add_duration(int *array, int dataPoint)
    {
        ...
        if (length < Data::MAX_INPUT_SIZE - 2)
        {                              // Ensures there is space for the new data point.
    ...
    {
        for (int i = 0; i < 26; i++)
        { // Loops through alphabet[] array size
            if (strcmp_P(user_pattern, (char *)pgm_read_word(&(validPatterns[i]))) == 0)
